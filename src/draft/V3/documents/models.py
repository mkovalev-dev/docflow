...
class DocumentLink(Base, BasicFieldsMixin, RefDocumentMixin):
    """Связки с документов"""

    __tablename__ = "document_document_link"
    __document_backref__ = "links"
    __document_foreign_keys__ = "[DocumentLink.document_id]"

    link_type: Mapped[str] = mapped_column(
        Enum(
            DocumentLinkTypeEnum,
            name="document_link_type_enum",
            native_enum=True,
            validate_strings=True,
        ),
        nullable=False,
    ...
    target: Mapped["Document"] = relationship(
        "Document", foreign_keys=[target_id], back_populates="links_in"
    )
